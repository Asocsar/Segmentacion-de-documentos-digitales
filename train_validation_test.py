...
def select_dataset(metadata, mode, filename=None, directory=None, experiment=None):
    ...
    if experiment is not None:
        if experiment == 1:
            filename.sort(key= lambda x: int(os.path.splitext(x)[0].split('_')[1]))
            dataset = H5DF_Unique(directory, filename,
                                data_transforms=transforms,
                                phase=mode, metadata=metadata)
        elif experiment == 2:
            dataset = Parquet(os.path.join(directory, filename),
                                data_transforms=transforms,
                                phase=mode, metadata=metadata)

        elif experiment == 3:
            dataset = Petastorm(os.path.join(directory, filename),
                                data_transforms=transforms,
                                phase=mode, metadata=metadata)
...
def train_val_test_loop_onedataset(epoch, model, TensorLog, metadata, loss_func, optimizer, folders, rank, scheduler, Max_accuracy):
    for m, directory in enumerate(folders):
        ...
        loss_file = 0
        dataset = select_dataset(metadata, mode, all_files, directory, experiment=1)
        ...
        display_information_endFile(rank, time_start, correct, iteration_length) 
